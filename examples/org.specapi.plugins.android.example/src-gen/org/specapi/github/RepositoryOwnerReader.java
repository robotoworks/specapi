...
public class RepositoryOwnerReader extends JsonEntityReader<RepositoryOwner> {			
	
	public RepositoryOwnerReader(JsonEntityReaderProvider provider) {
		super(provider);
	}
	
	public void read(JsonReader reader, RepositoryOwner entity) throws IOException {
		reader.beginObject();
		...
		while(reader.hasNext()) {
			...
			else if(name.equals("site_admin")) {
				entity.setSiteAdmin(JsonReaderUtil.coerceNextBoolean(reader));
			}
			else {
				reader.skipValue();
			}
	...
	public void readList(JsonReader reader, List<RepositoryOwner> entities) throws IOException {
		reader.beginArray();
		
		while(reader.hasNext()) {
			RepositoryOwner item = new RepositoryOwner();
			read(reader, item);
			entities.add(item);
